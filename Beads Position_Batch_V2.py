...
def progressBar(value, endvalue,*args, bar_length=20):
    '''Generate the progress bar while not change the line'''
...
def GetBeadsPosition_V2(folder,Targetfile, window_size, Bead_xy,ExpFolder, **kwargs):
    '''This program is to calculate the beads positions for each frame in the image file'''
    #Count the total frames to process.
    ...
    for i in range(0,len(Targetfile)):
        ...
        for j in range(Image._image_count):
            ...
            BeadP_list.append([frame_count,Image[j].metadata['time'], Cent[0], Cent[1]])
    ...
    return pd.DataFrame(BeadP_list,columns=['Frames','DateTime', 'x-Center', 'y-Center'])
...
list_Samples = [
    ["D:\\NAS-TEMP_BE\\20201119\\P01\\", "20201119-BE-Ibidi-850585-P01"],
    ["D:\\NAS-TEMP_BE\\20201119\\P02\\", "20201119-BE-Ibidi-850585-P02"],
    ["D:\\NAS-TEMP_BE\\20201119\\P03\\", "20201119-BE-Ibidi-850585-P03"],
    ["D:\\NAS-TEMP_BE\\20201119\\P04\\", "20201119-BE-Ibidi-850585-P04"],
    ["D:\\NAS-TEMP_BE\\20201119\\P05\\", "20201119-BE-Ibidi-850585-P05"]
    #["D:\\NAS-TEMP_BE\\20201119\\P06\\", "20201119-BE-Ibidi-850585-P06"]
    ]

temp_time = time.time()
for i,SampleInfo in enumerate(list_Samples):
    BeadsPosition_batch(SampleInfo[0],SampleInfo[0],SampleInfo[1],checking=False)
print("\n The program cost total time: ", (time.time() - temp_time) / 3600, " Hr")
