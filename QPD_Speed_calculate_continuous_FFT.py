...
SampleName = '20200106-BE-137-300530-T1-P02-FFT-Con'
...
WindowSize = 1 #unit: seconds
ShiftSize = 0.03 #unit: seconds


window_Np = int(SampleRate*WindowSize)
Shift_Np = int(SampleRate*ShiftSize)
...
LoopSize = (len(PositionData)-window_Np)//Shift_Np
for i in range(LoopSize):
    Start_ind = i*Shift_Np
    End_ind = Start_ind + window_Np
    #print("index(start,End) : ",Start_ind," , ", End_ind)
    Temp_Pdata = PositionData[Start_ind:End_ind].reset_index(drop=True)
    TempSpeedSheet = BA.GetSpeed(Temp_Pdata, Steps_N=window_Np, Fps=SampleRate)
    #'''
    if i == 0:
